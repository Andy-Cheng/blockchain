...
class BlockChain {
  ...
  static NewBlock(transcation, prevBlockHash, prevheight) {
    const newBlock = new Block(prevheight + 1, prevBlockHash, new Date(), 20, 0, transcation, "");
    newBlock.setHash();
    return newBlock;
  }
...
class Pow {
  constructor(block) {
    ...
    const {
      Bits
    } = block; // Set Target. Target menas the number larger than maximum possible solution by 1 bit.
  ...
  Run() {
    const {
      _target
    } = this; // Try nonce form 0 to maxNonce

    for (let nonce = 0; nonce < _constant.maxNonce; ++nonce) {
