...
} = require('./multerOption'); //testing data


let hash = "baa744f2bd8dd2afd98e13de0c3056cf70faef7635efb265d96ffe4d2736cdf4";
let genesisHash = "93bfd81989ed6feff36abe0228878d62298fd6ffa6ec92e2c3abb4744b4edc5c"; // blockChain api
// Read a block
...
blockChainRouter.get('/read/block', multer.fields(blockReadOptions), async (req, res, next) => {
  ...
  const {
    ChainId,
    height
  } = req.body; // Find block ginven its heigth and chainId.
  ...
  await blockModel.findOne({
    ...
    if (err) {
      ...
      res.status(200).json({
        apiMsg: apiMsg.blockReadUnSuccessfully
      });
    }
...
blockChainRouter.post('/add/block', multer.fields(blockAddOptions), async (req, res, next) => {
  ...
  const {
    ChainId,
    Transcations
  } = req.body; // Find prevHash and prev Height
  ...
  await blockModel.findOne({
    ...
    height: -1
  }).select("height Hash").exec(cb1).then(async prevInfo => {
    ...
    await newBlockInstance.save(function (err) {
      if (err) {
        ...
        res.status(200).json({
          apiMsg: apiMsg.blockSavedUnSuccessfully
        });
      } else {
        console.log('New BlockInstance saved successfully');
        res.status(200).json({
          apiMsg: apiMsg.blockSavedSuccessfully
        });
      }
    });
  });
...
blockChainRouter.get('/read/chain', multer.fields(blockChainReadOptions), async (req, res, next) => {
  ...
  const {
    ChainId
  } = req.body; // Find chain given its ChainId(GenesisHash)

  const cb1 = (err, blocks) => {
    if (err) {
      console.log(`read chain err: ${err}`);
      res.status(200).json({
        apiMsg: apiMsg.blockChainReadSuccessfully
      });
    } else {
      res.status(200).json({
        blocksOnChain: blocks
      });
    }
  };
